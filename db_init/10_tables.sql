...
CREATE TABLE anime (
  ...
  studio VARCHAR(100) NOT NULL REFERENCES studio(name) ON DELETE CASCADE,
  ...
  genre VARCHAR(100) NOT NULL REFERENCES genre(name) ON DELETE CASCADE,
...
CREATE TABLE anime_name_locale (
  anime_id INT PRIMARY KEY REFERENCES anime(id) ON DELETE CASCADE,
  japanese_name TEXT,
  romaji_name TEXT
);
